...
class OrderResource extends Resource
{
    ...
    {
        $selectProducts = collect($get('orderProducts'))->filter(fn($item) => !empty($item['product_id']) && !empty($item['quantity']));
        $prices = Product::find($selectProducts->pluck('product_id'))->pluck('price','id');
        $total = $selectProducts->reduce(function ($total, $product) use ($prices){
    ...
    protected static function updateExchangePaid(Get $get, Set $set): void{
        $paidMount = (int) $get('paid_amount') ?? 0;
        $totalPrice = (int) $get('total_price') ?? 0;
        $exchangePaid = $paidMount - $totalPrice;
